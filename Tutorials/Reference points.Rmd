...
where $N_{t}$ is the number of adults at time *t*, $R_{t}$ is the number of recruits at time *t*, and $d_{t}$ is the fraction of recruits lost due to some source of mortliaty (i.e., harvest or disturbance). It then follows that $1-d_{t}$ is discrete survival.
...
So effectively the number of adults at time *t* is a function of the abundance of adults at a previous time-step *t-1* that survived some potential harvest regime. Let's assume $d=0$ to begin:
\begin{align}
N_{t}=(1-d)*\cfrac{{\alpha}N_{t-1}}{1+\cfrac{\alpha}{\beta}N_{t-1}}\epsilon_{t}
\end{align}
...
For example, take a two different populations that each vary in productivty $\alpha_1=2; \alpha_2=4$ and carrying capacity $\beta_1=100; \beta_2=200$. 
...
Based on these $\alpha$ and $\beta$ demographic parameters, we can use numerical methods to estimate various spawner, recruitment, or fisheries reference points. The most common ones are based on Maximum Sustainable Yield.
...
### MSY
Maximum Sustainable Yield *MSY* is the most fish (biomass or abundance) that can be removed from the population over the long-term given the productivity of the population. It is a very helpful thing to know about the population, and it has a clear numerical solution as it occurs at the *maximum* of a yield curve.
...
Then, we will estimate one version of a limit reference point called S(gen). S(gen) is the spawner abundance that, in the absence of fishing, can reach S(MSY) within 1 generation. It is a conservation-based benchmark and lets us know how low we can go before we need to close fisheries while still maintaining the ability to get back to MSY fairly quick.
...
$S_{MSY}=(\sqrt\alpha-1)/(\alpha-1)/\beta)$, which for our case is: `r (sqrt(5)-1)/((alpha-1)/beta)`

But this doesn't work for all models. And, in our case, numerical solution for $S_{MSY}$ (`r S_msy`) gives a pretty close approximation to the analytical solution (`r (sqrt(5)-1)/((alpha-1)/beta)`) and works in many, many situations.
...
abline(v=S_msy,lty=1,lwd=2,col="orange3")
abline(v=S_msy_est,lty=2,lwd=2,col="orange3")
...
Not bad! Now, we have our reference points. And, we can assess this population in the future and determine status like: is it undergoing overfishing $U_{t}>U_{MSY}$ or it overfished $S_t<S_{MSY}$. Or even, is it at a conservation concern $S_t<S_{GEN}$. Such is the power of the quantitative approach.
